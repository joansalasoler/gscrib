...
* Use `wait_for_*()` methods to block until temperature reached.
...
| M60 | {meth}`pallet_exchange() <gscrib.GCodeBuilder.pallet_exchange>` | Pallet exchange. |
| M109 | {meth}`wait_for_hotend(temperature, heating_only) <gscrib.GCodeBuilder.wait_for_hotend>` | Wait to reach temperature. |
| M190 | {meth}`wait_for_bed(temperature, heating_only) <gscrib.GCodeBuilder.wait_for_bed>` | Wait to reach temperature. |
| M191 | {meth}`wait_for_chamber(temperature, heating_only) <gscrib.GCodeBuilder.wait_for_chamber>` | Wait to reach temperature. |
...
| M60 | {meth}`halt("pallet-exchange") <gscrib.GCodeBuilder.halt>` | Alias {meth}`pallet_exchange() <gscrib.GCodeBuilder.pallet_exchange>` |
| M109 | {meth}`halt("wait-for-hotend") <gscrib.GCodeBuilder.halt>` | Alias {meth}`wait_for_hotend() <gscrib.GCodeBuilder.wait_for_hotend>` |
| M190 | {meth}`halt("wait-for-bed") <gscrib.GCodeBuilder.halt>` | Alias {meth}`wait_for_bed() <gscrib.GCodeBuilder.wait_for_bed>` |
| M191 | {meth}`halt("wait-for-chamber") <gscrib.GCodeBuilder.halt>` | Alias {meth}`wait_for_chamber() <gscrib.GCodeBuilder.wait_for_chamber>` |
...
| G-code | Gscrib |
|--------|--------|
| G38.2 / G38.3 | {meth}`probe_towards(point, error_on_miss) <gscrib.GCodeBuilder.probe_towards>` |
| G38.4 / G38.5 | {meth}`probe_away(point, error_on_miss) <gscrib.GCodeBuilder.probe_away>` |
